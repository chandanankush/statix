...
- **Forwarder (`system-stats-forwarder`)** — Console script that periodically polls the FastAPI endpoint and forwards condensed metrics (`cpu`, `ram`, `swap_percent`, `disk`, `disk_read`, `disk_write`, `net_read`, `net_write`) alongside the rich snapshot to the monitoring server. Supports multiple destinations via comma-separated `MONITORING_SERVER_METRICS_URL`; each destination is contacted independently.
...
- **Dashboard** — Chart.js-powered page (`server/templates/dashboard.html`) that:
  - Renders live trend charts (CPU %, RAM % + Swap %, Disk I/O, Network I/O) with drag-to-reorder.

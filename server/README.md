...
| `GET` | `/data` | No | Query stored metrics. Params: `hostname`, `timeframe` (`1h`, `24h`, `7d`). Each point includes `cpu`, `ram`, `swap_percent`, `disk`, `disk_read`, `disk_write`, `net_read`, `net_write`, `load1`, `load5`, `load15` (nullable), `cpu_cores` (list of per-core %, nullable). |
